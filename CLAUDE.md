...
Finnhub news/sentiment: `async_news_sentiment_and_volume()` computes date range dynamically per call (not at module load) so long-running servers always query the current 7-day window. Logs a startup warning if `FINNHUB_API_KEY` is missing and skips Finnhub calls entirely (returns 0) rather than sending `token=None`. Auth errors (401/403) are logged separately from rate limits. After each scan, a summary logs how many tickers had news and the top 3 by article count.
